...
/**
 * Cette classe implémente un serveur TCP qui accepte en permanence des connexions, et pour chacune, envoie
 * au client le message « Bienvenue sur mon serveur et au revoir » avant de rompre immédiatement la communication.
 * Exemple d'utilisation : java exo1.Server 2021 ou java exo1.Server
 * 
...
public class Server {
     ...
     * Elle prend au maximum un argument si ce nombre est supérieur à 1 le programme s'arrête et affiche un message.
    ...
    public static void main(String[] argv) {
        ...
        //Annonce à l'administrateur que le serveur a bien démarré
        System.out.println("Le serveur écoute sur le port : " + port);
        
        //Traitements des requêtes 
        while (true) {
            Socket socketClient = createSocketClient(server);
            cptConnexion++;

            //Affichage et sauvegarde des logs dans un fichier texte
            String logs = createLogs(socketClient,cptConnexion);
            saveLogsOnFile(logs);
            ...
            //rupture de la connexion avec le client
    ...
    private static int getPort(String [] argv) {
        ...
        try {
            ...
            String msg ="Numéro de port incorrect";
            saveLogsOnFile(msg);
            System.exit(1);
        }
        return port;
    ...
    /**
    * Cette méthode permet d'envoyer un message au client.
    ...
    */
    private static void sendMessage(Socket socketClient){
        try{
            ...
            write.println("Bienvenue sur mon serveur et au revoir");
        }
        catch(IOException e){
    ...
    */
    private static String createLogs(Socket socketClient,int cptConnexion){
        ...
        try{
            ...
            logs = socketClient.getInetAddress().getHostName() + " s'est connecté à " + formatDate.format(new Date());
            logs+="\nNombre de connexion total : "+cptConnexion;
        }
        catch(Exception e){
     ...
     * La longueur maximale de la file d'attente pour les indications de connexion entrantes (une demande de connexion) est fixée à 50.
     * Si une indication de connexion arrive alors que la file d'attente est pleine, la connexion est refusée.
    ...
    private static void saveLogsOnFile(String logs){
        ...
        String pathfile = "exo1/connection.txt";
        try{
    ...
    /**
     * Écoute pour une connexion à cette socket et l'accepte. La méthode se bloque jusqu'à ce qu'une connexion soit établie.
    ...
    /**
     * cette fonction permet de fermer une socket.
    ...
    /**
    * Cette méthode permet de suspendre le programme pendant x seconde
    * @param milliseconde le nombre de seconde à attendre
    */
    private static void wait(int milliseconde){
        try{
            System.out.println("Traitement de la requête");
            Thread.sleep(milliseconde*1000);
        }
        catch(Exception e){
            System.out.println("Erreur est survenue lors de la pause du programme");
        }
    }
}